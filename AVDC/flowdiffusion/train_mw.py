...
def main(args):
    ...
    if args.mode == 'train':
        ...
        root, _ = splitext(args.inference_path)
        out_dir = root + '_frames'
        os.makedirs(out_dir, exist_ok=True)
        for idx, frame in enumerate(output):
            Image.fromarray(frame).save(os.path.join(out_dir, f'{idx:03d}.png'))

        print(f'Saved {len(output)} frames to {out_dir}')
